import { parseQueryString, stringfyQueryString } from '..'
...
describe('urlQuery: ', () => {
  ...
  describe('stringfyQueryString', () => {
    it(`stringfyQueryString({name: 'ryo', job: 'fe'}) should return name=ryo&job=fe`, () => {
      expect(stringfyQueryString({ name: 'ryo', job: 'fe' })).toBe('name=ryo&job=fe')
    })
    it(`stringfyQueryString() should return 'name=ryo&job%5B0%5D=1&job%5B1%5D=2'`, () => {
      expect(stringfyQueryString({ name: 'ryo', job: [1, 2] })).toBe('name=ryo&job%5B0%5D=1&job%5B1%5D=2')
    })
  })
})
